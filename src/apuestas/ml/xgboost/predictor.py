...
from apuestas.ml.xgboost.data_loader import load_historical_matches, load_matches
from apuestas.ml.xgboost.odds_utils import devig, VALUE_THRESHOLD
from apuestas.ml.xgboost.feature_engineer import build_match_features
from apuestas.ml.xgboost.model import XGBoostResult
from apuestas.infrastructure.telegram import TelegramNotifier, generate_match_card
from apuestas.infrastructure.groq import generate_match_explanation
...
class H2HProvider(Protocol):
    def get_h2h(self, betplay_event_id: int, home_team: str, away_team: str) -> dict | None: ...
...
def run(db_name: str, h2h_service: H2HProvider):
